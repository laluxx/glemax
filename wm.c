...
void other_window() {
    ...
    if (target && target != selected_frame->wm.selected) {
        ...
        // Use set_buffer instead of direct assignment
        set_buffer(target->buffer);
...
void restore_window_configuration(WindowConfiguration *config) {
    ...
    set_buffer(selected_frame->wm.selected->buffer);
...
void delete_window() {
    ...
    if (new_selected) {
      ...
      set_buffer(new_selected->buffer);
...
// TODO it doesn't draw the right edge of the box
// when there is at least one vertical split
...
void update_windows_scroll() {
    ...
    // TODO Also update minibuffer if active
    // get minibuffer scrolling right.
    /* if (selected_frame->wm.minibuffer_active && */
    /*     selected_frame->wm.minibuffer_window) { */
    /*     update_window_scroll(selected_frame->wm.minibuffer_window); */
    /* } */
}
...
float calculate_minibuffer_height() {
    ...
    // Check resize-mini-windows setting
    // nil (#f) = don't resize
    // t (#t) = resize to fit text (can grow and shrink freely)
    // 'grow-only (default) = grow to fit, but never shrink (until closed/empty)
    SCM resize_var = scm_c_lookup("resize-mini-windows");
    bool should_resize = true;
    bool grow_only = true;  // Default is 'grow-only

    if (!scm_is_false(resize_var)) {
        SCM resize_val = scm_variable_ref(resize_var);

        if (scm_is_false(resize_val)) {
            // nil = don't resize at all
            return selected_frame->wm.minibuffer_window->height;
        } else if (scm_is_true(resize_val) && scm_is_bool(resize_val)) {
            // t = resize to fit text freely (can grow and shrink)
            grow_only = false;
        } else if (scm_is_symbol(resize_val)) {
            // Check if it's 'grow-only
            SCM grow_only_symbol = scm_from_utf8_symbol("grow-only");
            if (scm_is_true(scm_equal_p(resize_val, grow_only_symbol))) {
                grow_only = true;
            }
        }
    }
    ...
    float usable_width = selected_frame->wm.minibuffer_window->width -
                        (selected_frame->left_fringe_width + selected_frame->right_fringe_width);

    size_t text_len = rope_char_length(buf->rope);

    // Reset max_height_reached when buffer becomes empty
    if (text_len == 0) {
        max_height_reached = 0.0f;
        return selected_frame->line_height;  // Return to 1 line when empty
    }
    ...
    if (!scm_is_false(max_var)) {
        SCM max_val = scm_variable_ref(max_var);

        if (scm_is_real(max_val)) {
            if (scm_is_inexact(max_val)) {
                // Inexact number (float): fraction of frame height
                float fraction = (float)scm_to_double(max_val);
                max_height = context.swapChainExtent.height * fraction;
            } else {
                // Exact number (integer): number of lines
                int max_lines = scm_to_int(max_val);
                max_height = max_lines * selected_frame->line_height;
            }
        } else {
            // Invalid type, use default
            max_height = context.swapChainExtent.height * 0.25f;
        }
    } else {
        // Variable not found, use default (25% of frame)
        max_height = context.swapChainExtent.height * 0.25f;
    }

    // Apply max-mini-window-height constraint
    if (desired_height > max_height) {
        desired_height = max_height;
    }

    // Calculate minimum height for windows above minibuffer
    size_t min_lines = scm_get_size_t("window-min-height", 4);
    float min_window_height = (float)min_lines * selected_frame->line_height;

    // Calculate maximum allowed minibuffer height without violating window-min-height
    float frame_height = context.swapChainExtent.height;
    float max_allowed_minibuffer_height = frame_height - min_window_height;

    // Ensure minibuffer doesn't push windows above their minimum
    if (desired_height > max_allowed_minibuffer_height) {
        desired_height = max_allowed_minibuffer_height;
    }

    // Ensure we return at least one line height
    if (desired_height < selected_frame->line_height) {
        desired_height = selected_frame->line_height;
    }

    // Apply 'grow-only behavior: never shrink below max reached
    if (grow_only) {
        if (desired_height > max_height_reached) {
            max_height_reached = desired_height;
        }
