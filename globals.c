...
float  minimap_width             = 110;   // TODO Option to lerp this in up to 110 from 0 with an easing function

bool   minimap_easing_mode       = true;  // Enable easing animation when toggling minimap
float  minimap_target_width      = 110.0f; // Target width for minimap easing
bool   minimap_lerp_active       = false;  // Whether minimap width is currently being animated

bool   minimap_detailed_mode     = true;  
...
bool   minibuffer_minimap_mode = false;

// NOTE Turning an Undefined behavior into a feature
// if false, it will randomly do buffer creation transition
// if true, try to fix the problem, in the wrong way.
bool   SkipFistScrollLerpFrameUpdate = false; // HACK

bool   scroll_bar_mode = true;
size_t scroll_bar_thickness = 8;

bool keep_right_fringe = false; // TODO update drawFringe..
