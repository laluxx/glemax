...
(define (minor-mode-info-lighter info)      (assq-ref info 'lighter))
(define (minor-mode-info-keymap info)       (assq-ref info 'keymap))
(define (minor-mode-info-enable-hook info)  (assq-ref info 'enable-hook))
(define (minor-mode-info-disable-hook info) (assq-ref info 'disable-hook))
(define (minor-mode-info-global? info)      (assq-ref info 'global?))
  ...
  (syntax-rules ()
    ((_ mode doc-string . options)
     (begin
       ...
       (define mode-keymap-sym   (assq-ref mode-options 'keymap))
...
(define (parse-minor-mode-options options)
  ...
  (let loop ((opts options) (result '()))
    (if (null? opts)
        result
        (let ((key (car opts)))
          (cond
           ((eq? key ':lighter)
            (loop (cddr opts) (acons 'lighter (cadr opts) result)))
           ((eq? key ':keymap)
            (loop (cddr opts) (acons 'keymap (cadr opts) result)))
           ((eq? key ':enable-hook)
            (loop (cddr opts) (acons 'enable-hook (cadr opts) result)))
           ((eq? key ':disable-hook)
            (loop (cddr opts) (acons 'disable-hook (cadr opts) result)))
           ((eq? key ':global)
            (loop (cddr opts) (acons 'global (cadr opts) result)))
           (else
...
;; (define (minor-mode-toggle mode-symbol arg body)
;;   "Toggle or set a minor mode. ARG can be 'toggle, a number, or a boolean."
;;   (let* ((info (get-minor-mode-info mode-symbol))
;;          (global? (and info (minor-mode-info-global? info)))
;;          (current-value
;;           (if global?
...
;;            ((number? arg)     (> arg 0))
;;            (else              arg))))
;;     (when (not (equal? (if current-value #t #f) (if new-value #t #f)))
;;       (if new-value
;;           (minor-mode-enable mode-symbol info global? body)
;;           (minor-mode-disable mode-symbol info global?)))
;;     new-value))
